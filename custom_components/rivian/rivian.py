...
class Rivian:
    ...
    async def authenticate(
        ...
        if response.status == 401 and response_json["error"] == "otp_entry_required":
            self._otp_needed = True
            self._session_token = response_json["session_token"]
            self.close()
            return {"message": "OTP Required! Check for a text message from Rivian.", "session_token": self._session_token}
    ...
    async def get_vehicle_info(
        ...
        if response.status == 401:
            await refresh_tokens(self._access_token)
